...
const FrontendLogger = ({ onComplete, onLogout, username = "User" }) => {
  ...
  const handleExerciseLog = async () => {
    ...
    if (!userProfile.weight || !userProfile.height) {
      alert("Weight and height are required to calculate BMI. Please update your profile.");
      return;
    }

    const weightLoss = (caloriesBurnt / 7700).toFixed(2);
    const updatedWeight = (parseFloat(userProfile.weight) - parseFloat(weightLoss)).toFixed(2);
    const updatedBMI = (updatedWeight / (userProfile.height / 100) ** 2).toFixed(2);
